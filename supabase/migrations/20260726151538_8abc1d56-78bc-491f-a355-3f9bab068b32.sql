-- Include 'lektor' in the teacher/admin check helper
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE user_id = auth.uid() AND role IN ('admin', 'teacher', 'lektor')
  )
$function$;

-- Map role_label='lektor' from signup metadata to the 'lektor' role
