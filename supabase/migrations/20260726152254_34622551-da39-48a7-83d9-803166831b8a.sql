  ...
  BEGIN
    ...
    BEGIN
      ...
      VALUES (
        ...
        CASE WHEN _role_label IN ('rodic', 'lektor') THEN '' ELSE COALESCE(NEW.raw_user_meta_data->>'school', '') END,
    ...
    EXCEPTION
      WHEN unique_violation THEN
        ...
        SET email = COALESCE(NEW.email, email),
            school = CASE WHEN _role_label IN ('rodic', 'lektor') THEN '' ELSE school END
        WHERE id = NEW.id;
      WHEN OTHERS THEN
    ...
    ELSIF _role_label = 'lektor' THEN
      _assigned_role := 'lektor';
    ...
    IF _role_label = 'teacher' AND _school_code IS NOT NULL AND _school_code != '' THEN
      BEGIN
...
BEGIN
  IF NOT (public.is_admin() OR public.is_school_admin_of(_school_id, auth.uid())) THEN
    RAISE EXCEPTION 'Not authorized to view license usage';
  END IF;
...
AS $function$
  SELECT
    p.school_id,
    COUNT(DISTINCT p.id) FILTER (WHERE ur.role = 'teacher')::int,
