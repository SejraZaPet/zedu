...
export default function WorksheetEditor() {
  ...
  /** ?generate=1 — rovnou otevře dialog generování z obsahu lekce. */
  const autoGenerateParam = searchParams.get("generate");
  ...
  function addItemFromLessonActivity(activity: LessonActivity) {
    if (!spec) return;
    const variantId = spec.variants[0].variantId;
    const mapped = buildItemsFromLessonActivity(activity);
    if (mapped.length === 0) return;

    const newItems: WorksheetItem[] = [];
    const newKeys: ReturnType<typeof createDefaultAnswerKey>[] = [];
    mapped.forEach((m, i) => {
      const base = createDefaultItem(m.type, items.length + newItems.length + 1);
      const newItem: WorksheetItem = { ...base, ...m.patch };
      const key = createDefaultAnswerKey(newItem);
      newItems.push(newItem);
      newKeys.push(m.correct !== undefined ? { ...key, correctAnswer: m.correct } : key);
      void i;
    });
    ...
    toast({
      title:
        newItems.length > 1
          ? `Aktivita z lekce vložena (${newItems.length} otázek)`
          : "Aktivita z lekce vložena",
  ...
  /** Režim pro každou aktivitu z lekce: QR kód na aktivitu, nebo převod na blok listu. */
  const [activityModes, setActivityModes] = useState<Record<string, "qr" | "convert">>({});
  const topicPrefillDone = useRef(false);
  /** Kontext předaný z „Návrh lekce podle metody“ (modelová situace + aktivity fází). */
  const [suggestionContext, setSuggestionContext] = useState("");
  const autoGenerateDone = useRef(false);

  // Výchozí režim aktivit = QR kód
  useEffect(() => {
    setActivityModes((prev) => {
      const next: Record<string, "qr" | "convert"> = {};
      for (const a of activeLessonActivities) next[a.id] = prev[a.id] ?? "qr";
      return next;
    });
  }, [activeLessonActivities]);

  // Odkaz na přiřazenou lekci — potřebný pro QR kódy na aktivity.
  useEffect(() => {
    if (!activeLessonId) {
      setLessonLinkBase(null);
      return;
    }
    let cancelled = false;
    (async () => {
      const type = allLessons.find((l) => l.id === activeLessonId)?.type ?? "global";
      const linked = await resolveLinkedLesson(
        activeLessonId,
        type === "teacher" ? "teacher_textbook_lessons" : "textbook_lessons",
      );
      if (!cancelled) {
        setLessonLinkBase(linked ? `${window.location.origin}${linked.url}` : null);
      }
    })();
    return () => {
      cancelled = true;
  ...
  /** URL na konkrétní aktivitu v lekci (deep-link ?aktivita=N). */
  function activityUrlFor(deepLinkIndex: number): string | null {
    if (!lessonLinkBase) return null;
    const sep = lessonLinkBase.includes("?") ? "&" : "?";
    return `${lessonLinkBase}${sep}aktivita=${deepLinkIndex}`;
  }

  // Když učitel přišel přes „Vygenerovat pracovní list z této lekce“, otevřeme
  // panel sekcí (sekce po sekci); bez sekcí padáme na klasické AI generování.
  useEffect(() => {
    if (autoGenerateParam !== "1" || autoGenerateDone.current) return;
    if (!activeLessonContent || activeLessonContent.trim().length < 20) return;
    autoGenerateDone.current = true;
    if (activeLessonSections.length > 0) setSectionsPanelOpen(true);
    else setShowAiGenerateDialog(true);
  }, [autoGenerateParam, activeLessonContent, activeLessonSections]);
  ...
  useEffect(() => {
    ...
    setAiMode("study");
    let suggestionPrefill = "";
    try {
      suggestionPrefill = (id && sessionStorage.getItem(`bezli.worksheetPrefill:${id}`)) || "";
    } catch {
      /* ignore */
    }
    if (suggestionPrefill) setSuggestionContext(suggestionPrefill);
    setAiCustomHint((prev) => {
      if (prev) return prev;
      if (suggestionPrefill) {
        return `Pracovní list k hodině „${topicParam}“ navržené podle výukové metody. Vycházej z modelové situace a aktivit z fází hodiny (viz kontext).`;
      }
  ...
  }, [topicParam, topicRocnikParam, topicFocusParam, id]);
